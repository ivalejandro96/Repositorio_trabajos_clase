console.log("Ahora hablemos de bucles");
...
/*
    Ciclo for
    Un ciclo for se repite hata que la condicion se evalue como falsee. Crea 3 expresiones opcionales
    Sintaxis 
...
*/
    //Se crea un programa para saber cuantos boletos vendemos para una rifa

for (let boleto = 1; boleto <= 15; boleto++){
        console.log(`boleto # ${boleto}`);
    }   
    //Programa para vender regalos con una variable global
let regalos = 1;
    for (regalos;regalos<=20;regalos++){
        console.log(`vendi ${regalos}`);
    }
    //Cuenta regresiva
let cuentaRegresiva = 15;
    for(cuentaRegresiva;cuentaRegresiva >=0;cuentaRegresiva--){
        console.log(`Quedan ${cuentaRegresiva} regalos`);
    }
/*
    Reconocer arrays con el ciclo for
    Debemos usar indices del arreglo para poder usar el bucle, en indices desde el 0 hasta n y usaremos la propidad length
*/
const ch35 = ["Diana","Monse","Ania","Ceci"];
...
let productos = 5;
    while (productos > 0){
        console.log(productos+"productos vendidos");productos--;
}
//Imprimir numeros
let num1 = 0;
    while(num1<10){
